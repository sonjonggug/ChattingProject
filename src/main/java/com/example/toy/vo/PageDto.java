...
@Getter
@Setter
public class PageDto { // 화면에 그려질 pageNation을 계산하는 클래스 ( pageNum, pageSize값을 가지고 다님 )
...
//		 ex) 조회하는 페이지 11 -> 끝번호 20
        this.endPage = (int) Math.ceil(this.pageNum * 0.1) * 10; // 공식 = (int)Math.ceil(페이지번호 / 페이지네이션개수) * 페이지네이션개수
        this.startPage = this.endPage - 10 + 1; // 공식 = 끝페이지 - 페이지네이션개수 + 1
...
//		 11 클릭시 endPage = 14 , realEnd = 14 -> 다음버튼 false
        ...
        System.out.println("pageNum:" + this.pageNum + ", pageSize:" + this.pageSize);
